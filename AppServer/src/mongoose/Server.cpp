...
static int iterate_callback(struct mg_connection *connection) {
	if (connection->is_websocket && connection->content_len) {
		Server *server = (Server *) connection->server_param;
		server->_webSocketData(connection,
				string(connection->content, connection->content_len));
	}
...
Server::Server(int port, const char *documentRoot) :
		stopped(false), destroyed(false), server(NULL)
#ifndef NO_WEBSOCKET 
				, websockets(NULL)
#endif
...
void Server::printStats() {
	...
	if (delta) {
		cout << "Requests: " << requests << ", Requests/s: "
				<< (requests * 1.0 / delta) << endl;
	}
}
}
