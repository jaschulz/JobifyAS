...
bool Request::hasVariable(string key) {
	...
	return dataField != NULL
			&& mg_get_var(connection, key.c_str(), dummy, 1) != -1;
}
...
bool Request::hasCookie(string key) {
	...
	for (i = 0; i < connection->num_headers; i++) {
		...
		if (strcmp(header->name, "Cookie") == 0) {
			if (mg_get_cookie(header->value, key.c_str(), dummy, sizeof(dummy))
					!= -1) {
				return true;
			}
		}
...
void Request::handleUploads() {
	...
	if (mg_parse_multipart(connection->content, connection->content_len,
			var_name, sizeof(var_name), file_name, sizeof(file_name), &data,
			&data_len)) {
		uploadFiles.push_back(
				UploadFile(string(file_name), string(data, data_len)));
	}
}
