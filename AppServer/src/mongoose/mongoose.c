...
#ifndef NO_FILESYSTEM
	"access_log_file", NULL, "auth_domain", "mydomain.com", "cgi_interpreter",
	NULL, "cgi_pattern", "**.cgi$|**.pl$|**.php$", "dav_auth_file", NULL,
	"document_root", NULL, "enable_directory_listing", "yes",
#endif
...
} static_builtin_mime_types[] = { { ".html", 5, "text/html" }, { ".htm", 4,
	"text/html" }, { ".shtm", 5, "text/html" }, { ".shtml", 6, "text/html" }, {
	".css", 4, "text/css" }, { ".js", 3, "application/x-javascript" }, { ".ico",
	4, "image/x-icon" }, { ".gif", 4, "image/gif" },
	{ ".jpg", 4, "image/jpeg" }, { ".jpeg", 5, "image/jpeg" }, { ".png", 4,
			"image/png" }, { ".svg", 4, "image/svg+xml" }, { ".txt", 4,
			"text/plain" }, { ".torrent", 8, "application/x-bittorrent" }, {
			".wav", 4, "audio/x-wav" }, { ".mp3", 4, "audio/x-mp3" }, { ".mid",
			4, "audio/mid" }, { ".m3u", 4, "audio/x-mpegurl" }, { ".ogg", 4,
			"application/ogg" }, { ".ram", 4, "audio/x-pn-realaudio" }, {
			".xml", 4, "text/xml" }, { ".json", 5, "text/json" }, { ".xslt", 5,
			"application/xml" }, { ".xsl", 4, "application/xml" }, { ".ra", 3,
			"audio/x-pn-realaudio" }, { ".doc", 4, "application/msword" }, {
			".exe", 4, "application/octet-stream" }, { ".zip", 4,
			"application/x-zip-compressed" },
	{ ".xls", 4, "application/excel" }, { ".tgz", 4, "application/x-tar-gz" }, {
			".tar", 4, "application/x-tar" },
	{ ".gz", 3, "application/x-gunzip" }, { ".arj", 4,
			"application/x-arj-compressed" }, { ".rar", 4,
			"application/x-arj-compressed" }, { ".rtf", 4, "application/rtf" },
	{ ".pdf", 4, "application/pdf" }, { ".swf", 4,
			"application/x-shockwave-flash" }, { ".mpg", 4, "video/mpeg" }, {
			".webm", 5, "video/webm" }, { ".mpeg", 5, "video/mpeg" }, { ".mov",
			4, "video/quicktime" }, { ".mp4", 4, "video/mp4" }, { ".m4v", 4,
			"video/x-m4v" }, { ".asf", 4, "video/x-ms-asf" }, { ".avi", 4,
			"video/x-msvideo" }, { ".bmp", 4, "image/bmp" }, { ".ttf", 4,
			"application/x-font-ttf" }, { NULL, 0, NULL } };
...
if ((s = getenv(name)) != NULL)
	addenv(blk, "%s=%s", name, s);
}

static void prepare_cgi_environment(struct connection *conn, const char *prog,
...
addenv(blk, "REQUEST_URI=%s%s%s", ri->uri, ri->query_string == NULL ? "" : "?",
...
for (i = 0; i < ri->num_headers; i++) {
	p = addenv(blk, "HTTP_%s=%s", ri->http_headers[i].name,
...
if (start_process(conn->server->config_options[CGI_INTERPRETER], prog, blk.buf,
		blk.vars, dir, fds[1]) > 0) {
...
int len, s_len = sizeof(cgi_status) - 1, n = recv(conn->endpoint.cgi_sock, buf,
		sizeof(buf), 0);
	...
	if (conn->flags & CONN_BUFFER) {
		...
		if (len > 0) {
			...
			if (mg_get_header(&c, "Location") != NULL) {
				status = "302";
			} else if ((status = (char *) mg_get_header(&c, "Status"))
					== NULL) {
				status = "200";
			}
		}
...
if ((sscanf(spec, "%d.%d.%d.%d/%d%n", &a, &b, &c, &d, &slash, &n) == 5
		...
		&& isbyte(b) && isbyte(c) && isbyte(d) && slash >= 0 && slash < 33) {
	len = n;
...
if ((sock = accept(server->listening_sock, &sa.sa, &len)) == INVALID_SOCKET) {
	...
	sockaddr_to_string(conn->mg_conn.remote_ip, sizeof(conn->mg_conn.remote_ip),
			&sa);
	conn->mg_conn.remote_port = ntohs(sa.sin.sin_port);
	conn->mg_conn.server_param = server->server_data;
	LINKED_LIST_ADD_TO_FRONT(&server->active_connections, &conn->link); DBG(("added conn %p", conn));
}
...
if ((or_str = (const char *) memchr(pattern, '|', pattern_len)) != NULL) {
	...
	return res > 0 ?
			res :
			match_prefix(or_str + 1, (pattern + pattern_len) - (or_str + 1),
					str);
}
...
struct vec a, b;
const char *rewrites = conn->server->config_options[URL_REWRITES], *root =
		conn->server->config_options[DOCUMENT_ROOT], *cgi_pat =
		conn->server->config_options[CGI_PATTERN], *uri = conn->mg_conn.uri;
char *p;
int match_len;
...
while ((rewrites = next_option(rewrites, &a, &b)) != NULL) {
	if ((match_len = match_prefix(a.ptr, a.len, uri)) > 0) {
		mg_snprintf(buf, buf_len, "%.*s%s", (int) b.len, b.ptr,
				uri + match_len);
		break;
	}
}
...
for (i = 0; i < 20; i++) {
	digest[i] = (unsigned char) ((context->state[i >> 2] >> ((3 - (i & 3)) * 8))
			& 255);
}
...
mg_snprintf(buf, sizeof(buf), "%s%s%s", "HTTP/1.1 101 Switching Protocols\r\n"
		"Upgrade: websocket\r\n"
		"Connection: Upgrade\r\n"
...
int i, len, buf_len = conn->local_iobuf.len, frame_len = 0, mask_len = 0,
		header_len = 0, data_len = 0, buffered = 0;
...
int mg_websocket_write(struct mg_connection* conn, int opcode, const char *data,
	size_t data_len) {
unsigned char *copy;
size_t copy_len = 0;
int retval = -1;
...
if (data_len < 126) {
	...
	*(uint16_t *) (copy + 2) = (uint16_t) htons((uint16_t) data_len);
	...
	// 64-bit length field
	copy[1] = 127;
	*(uint32_t *) (copy + 2) = (uint32_t) htonl(
			(uint32_t)((uint64_t) data_len >> 32));
...
const char *ver = mg_get_header(conn, "Sec-WebSocket-Version"), *key =
...
for (i = 0; static_builtin_mime_types[i].extension != NULL; i++) {
	...
	if (path_len > static_builtin_mime_types[i].ext_len
			&& mg_strcasecmp(ext, static_builtin_mime_types[i].extension)
					== 0) {
...
LINKED_LIST_FOREACH(&server->uri_handlers, lp, tmp)
{
	...
	if (!strncmp(uh->uri, uri, strlen(uh->uri)))
		return uh;
}
...
static const unsigned short days_before_month[] = { 0, 31, 59, 90, 120, 151,
...
if (((sscanf(datetime, "%d/%3s/%d %d:%d:%d", &day, month_str, &year, &hour,
		&minute, &second) == 6)
		|| (sscanf(datetime, "%d %3s %d %d:%d:%d", &day, month_str, &year,
				&hour, &minute, &second) == 6)
		|| (sscanf(datetime, "%*3s, %d %3s %d %d:%d:%d", &day, month_str, &year,
				&hour, &minute, &second) == 6)
		|| (sscanf(datetime, "%d-%3s-%d %d:%d:%d", &day, month_str, &year,
				&hour, &minute, &second) == 6)) && year > 1970 && (month =
		get_month_index(month_str)) != -1) {
...
static int find_index_file(struct connection *conn, char *path, size_t path_len,
file_stat_t *stp) {
...
do {
} while (deliver_websocket_frame(conn));
} else
#endif
...
if (arr_ind >= arr_size) {
if ((p = (struct dir_entry *) realloc(*arr, (inc + arr_size) * sizeof(**arr)))
		!= NULL) {
...
n = mg_snprintf(chunk, sizeof(chunk), "<tr><td><a href=\"%s%s%s\">%s%s</a></td>"
"<td>&nbsp;%s</td><td>&nbsp;&nbsp;%s</td></tr>\n", de->conn->mg_conn.uri, href,
slash, de->file_name, slash, mod, size);
...
const struct dir_entry *a = (const struct dir_entry *) p1, *b =
(const struct dir_entry *) p2;
const char *qs =
a->conn->mg_conn.query_string ? a->conn->mg_conn.query_string : "na";
int cmp_result = 0;
...
int i, num_entries, sort_direction =
conn->mg_conn.query_string != NULL && conn->mg_conn.query_string[1] == 'd' ?
'a' : 'd';

conn->mg_conn.status_code = 200;
mg_snprintf(buf, sizeof(buf), "%s", "HTTP/1.1 200 OK\r\n"
"Transfer-Encoding: Chunked\r\n"
"Content-Type: text/html; charset=utf-8\r\n\r\n");
spool(&conn->remote_iobuf, buf, strlen(buf));

mg_snprintf(buf, sizeof(buf), "<html><head><title>Index of %s</title>"
"<style>th {text-align: left;}</style></head>"
"<body><h1>Index of %s</h1><pre><table cellpadding=\"0\">"
"<tr><th><a href=\"?n%c\">Name</a></th>"
"<th><a href=\"?d%c\">Modified</a></th>"
"<th><a href=\"?s%c\">Size</a></th></tr>"
"<tr><td colspan=\"3\"><hr></td></tr>", conn->mg_conn.uri, conn->mg_conn.uri,
...
const char *depth = mg_get_header(&conn->mg_conn, "Depth"), *list_dir =
conn->server->config_options[ENABLE_DIRECTORY_LISTING];
...
mg_snprintf(buf, sizeof(buf), "%s%s", de->conn->mg_conn.uri, de->file_name);
...
return -1;
if (p[1] == '\0')
return 0;  // Path is a directory itself
}
...
#else
} else if ((conn->endpoint.fd = open(path, O_RDWR | O_CREAT | O_TRUNC, 0644))
< 0) {
#endif
...
"realm=\"%s\", nonce=\"%lu\"\r\n\r\n",
conn->server->config_options[AUTH_DOMAIN], (unsigned long) time(NULL));
}
...
mg_md5(expected_response, ha1, ":", nonce, ":", nc, ":", cnonce, ":", qop, ":",
ha2, NULL);
...
|| mg_strncasecmp(hdr, "Digest ", 7) != 0)
return 0;
if (!mg_parse_header(hdr, "username", user, sizeof(user)))
return 0;
if (!mg_parse_header(hdr, "cnonce", cnonce, sizeof(cnonce)))
return 0;
if (!mg_parse_header(hdr, "response", resp, sizeof(resp)))
return 0;
if (!mg_parse_header(hdr, "uri", uri, sizeof(uri)))
return 0;
if (!mg_parse_header(hdr, "qop", qop, sizeof(qop)))
return 0;
if (!mg_parse_header(hdr, "nc", nc, sizeof(nc)))
return 0;
if (!mg_parse_header(hdr, "nonce", nonce, sizeof(nonce)))
return 0;
...
return check_password(c->request_method, ha1, uri, nonce, nc, cnonce, qop, resp);
}
return 0;
}
...
return s && (!strcmp(s, "PUT") || !strcmp(s, "DELETE") || !strcmp(s, "MKCOL"));
}
#endif // NO_AUTH
...
if ((s == str || s[-1] == ' ') && s[n] == '=' && !memcmp(s, var_name, n))
break;
}
...
if (conn->server->do_i_handle == NULL
|| conn->server->do_i_handle(&conn->mg_conn)) {
...
if (!(conn->flags & CONN_CONNECTED)
	&& getsockopt(conn->client_sock, SOL_SOCKET, SO_ERROR, (char *) &ok, &len)
			< 0) {
...
break;
case EP_CGI:
closesocket(conn->endpoint.cgi_sock);
break;
default:
break;
}
...
conn->cl < (int64_t) sizeof(buf) ? (int) conn->cl : (int) sizeof(buf));
...
#endif
if (FD_ISSET(conn->client_sock, &write_set) && !(conn->flags & CONN_BUFFER)) {
	conn->last_activity_time = current_time;
...
int mg_get_var(const struct mg_connection *conn, const char *name, char *dst,
size_t dst_len) {
int len = get_var(conn->query_string,
conn->query_string == NULL ? 0 : strlen(conn->query_string), name, dst,
dst_len);
if (len < 0) {
...
parse_header(buf + n + cdl, ll - (cdl + 2), "filename", file_name,
file_name_len);
}
}
