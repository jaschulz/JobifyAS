...
void ProfileTest::profileToJSON() {
	...
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("email", "").asString(),
			profile.getEmail());
}
...
void ProfileTest::addRecommendation() {
	...
	CPPUNIT_ASSERT_EQUAL(prevRecommendations + 1,
			profile.getRecommendationsCount());
}
...
void ProfileTest::correctProfileConversion() {
	...
	double latitude = 0;
	double longitude = 0;
	Profile p(mail, first_name, last_name, p_pic, job_position, latitude,
			longitude);
	Json::Value jsonProfile = p.publicProfileToJSON();
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("email", "").asString(), mail);
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("first_name", "").asString(),
			first_name);
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("last_name", "").asString(),
			last_name);
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("pic", "").asString(), p_pic);
	CPPUNIT_ASSERT_EQUAL(jsonProfile.get("job_position", "").asString(),
			job_position);
